...
def main():
    ...
    for record in tqdm(records):
        ...
        for positive in record["positive"]:
            ...
            positive_content = all_articles_map[positive]
            positive_seq = convert_article(positive_content)

            for neg in negative:
                neg_content = all_articles_map[neg]
                neg_seq = convert_article(neg_content)
                examples.append([
                    np.array(query),
                    np.array(pad_article_flat(positive_seq, max_article_len=max_article_len)),
                    np.array(pad_article_flat(neg_seq, max_article_len=max_article_len)),
                    np.array([1, 1, 0])
                ])

    os.makedirs("output", exist_ok=True)
    with open("output/flat_triplet_training_data_bert.pkl", 'wb') as f:
